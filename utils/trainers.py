...
class ModelTrainer(ABC):
    ...
    def train_val(self, train_dl, val_dl, start_epoch, end_epoch, path_to_weights, path_to_checkpoint, device, loss_history=None, metric_history=None, progress_bar=True):
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            if progress_bar:
                print("Training")
            self.model.train()
            train_loss, train_metric = self.__loss_epoch(train_dl, device, True, progress_bar)
            ...
            if val_dl is not None:
                ...
                with torch.no_grad():
                    self.model.eval()
                    val_loss, val_metric = self.__loss_epoch(val_dl, device, False, progress_bar)
    ...
    def test(self, data_dl, device, progress_bar=True):
        with torch.no_grad():
            self.model.eval()
            loss, metric = self.__loss_epoch(data_dl, device, False, progress_bar)
    ...
    def __loss_epoch(self, data_dl, device, train=True, progress_bar=True):
        def process(data, running_loss, running_metric):
            ...
            params.clear()
            loss_b = self.__loss_batch(output, data["y"], train)
            with torch.no_grad():
    ...
    def __loss_batch(self, output, target, train=True):
        loss = self.loss_func(output, target)
        if self.optimizer is not None and train:
...
class BasicTrainer(ModelTrainer):
    def __init__(self, model, opt, loss_func, lr_scheduler):
        super(BasicTrainer, self).__init__(model, opt, loss_func, lr_scheduler)
